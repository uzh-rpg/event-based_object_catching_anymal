...
//! simplified computation of numerical derivatives to verify the analytical
//! ones and in the optimizer. These traits are inspired by GTSAM and for
//! rotations and transformation we use the same convention (updates are applied
//! on the right hand side / in the body frame).
...
template <typename Scalar> struct ScalarTraits {
  enum {
    dimension = 1
  }; //! @todo(cfo): static constexpr int fails on GCC 4.8, works for Eigen
     //! because no derived class.
  typedef Eigen::Matrix<Scalar, 1, 1> TangentVector;
  typedef Eigen::Matrix<Scalar, 1, 1> Jacobian;

  static int getDimension(const Scalar /*v*/) { return 1; }
...
// -----------------------------------------------------------------------------
// Manifold traits for fixed-size Eigen matrices and vectors with double
// precision.
...
struct traits<Eigen::Matrix<real_t, M, N, Options, MaxRows, MaxCols>> {
  ...
  static int getDimension(const Matrix & /*v*/) { return M * N; }
...
// -----------------------------------------------------------------------------
// Manifold traits for dynamic-size Eigen matrices and vectors with double
// precision.
namespace internal {
...
struct DynamicMatrixTraits {
  ...
  static TangentVector local(const DynamicMatrix &origin,
                             const DynamicMatrix &other, Jacobian *H1 = nullptr,
                             Jacobian *H2 = nullptr) {
    if (H1) {
    ...
    Eigen::Map<DynamicMatrix>(v.data(), origin.rows(), origin.cols()) =
        other - origin;
    return v;
  }

  static DynamicMatrix retract(const DynamicMatrix &origin,
                               const TangentVector &v, Jacobian *H1 = nullptr,
                               Jacobian *H2 = nullptr) {
    if (H1) {
    ...
    return origin + Eigen::Map<const DynamicMatrix>(v.data(), origin.rows(),
                                                    origin.cols());
  }
};
...
struct traits<Eigen::Matrix<real_t, Eigen::Dynamic, Eigen::Dynamic, Options,
                            MaxRows, MaxCols>>
    : public internal::DynamicMatrixTraits<Eigen::Dynamic, Eigen::Dynamic,
                                           Options, MaxRows, MaxCols> {};

// traits for dynamic column vector
template <int Options, int MaxRows, int MaxCols>
struct traits<
    Eigen::Matrix<real_t, Eigen::Dynamic, 1, Options, MaxRows, MaxCols>>
    : public internal::DynamicMatrixTraits<Eigen::Dynamic, 1, Options, MaxRows,
                                           MaxCols> {};

// traits for dynamic row vector
template <int Options, int MaxRows, int MaxCols>
struct traits<
    Eigen::Matrix<real_t, 1, Eigen::Dynamic, Options, MaxRows, MaxCols>>
    : public internal::DynamicMatrixTraits<1, Eigen::Dynamic, Options, MaxRows,
                                           MaxCols> {};
...
template <> struct traits<Quaternion> {
  ...
  static int getDimension(const Quaternion & /*v*/) { return 3; }

  static bool equals(const Quaternion &q1, const Quaternion &q2,
                     real_t tol = 1e-8) {
    return (q1.getUnique().vector() - q2.getUnique().vector())
               .array()
               .abs()
               .maxCoeff() < tol;
  }
  ...
  static TangentVector local(const Quaternion &origin, const Quaternion &other,
    ...
    if (H1 || H2) {
      ...
      if (H1) {
        // dlocal(origin, other) / dorigin, using that Adjoint(h.inverse()) =
        // h.inverse()
  ...
  static Quaternion retract(const Quaternion &origin, const Vector3 &v,
                            Jacobian *H1 = nullptr, Jacobian *H2 = nullptr) {
    ...
    if (H1) {
      ...
      *H1 =
          g.inverse().getRotationMatrix(); // Adjoint(g.inverse()) = g.inverse()
    }
    if (H2) {
...
template <> struct traits<Transformation> {
  ...
  static int getDimension(const Transformation & /*v*/) { return 6; }

  static bool equals(const Transformation &T1, const Transformation &T2,
                     real_t tol = 1e-8) {
    return (T1.getRotation().getUnique().vector() -
            T2.getRotation().getUnique().vector())
                   .array()
                   .abs()
                   .maxCoeff() < tol &&
  ...
  static TangentVector local(const Transformation &origin,
                             const Transformation &other,
                             Jacobian *H1 = nullptr, Jacobian *H2 = nullptr) {
    const Transformation h = origin.inverse() * other;
    const TangentVector v =
        (Vector6() << h.getPosition(), h.getRotation().log()).finished();
    if (H1 || H2) {
      ...
      if (H1) {
        ...
        H1->block<3, 3>(3, 3) =
            -J_r_inv * h.getRotation().inverse().getRotationMatrix();
      }
      if (H2) {
        // dlocal(origin, other) / dother
  ...
  static Transformation retract(const Transformation &origin, const Vector6 &v,
                                Jacobian *H1 = nullptr,
                                Jacobian *H2 = nullptr) {
    ...
    if (H1 || H2) {
      const Matrix3 R_CB =
          Quaternion::exp(v.tail<3>()).inverse().getRotationMatrix();
      if (H1) {
