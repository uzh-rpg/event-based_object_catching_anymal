...
//! _ interpolate(Ringbuffer<...>*, int64_t time,
//! Ringbuffer<...>timering_t::iterator); Passing the (optional) interator to
//! the timestamp right before the to be interpolated value speeds up the
//! process. The passed it_before is expected to be valid.
//!
//! A nearest neighbour "interpolator".
struct InterpolatorNearest {
  template <typename Ringbuffer_T>
  static typename Ringbuffer_T::DataType
  interpolate(Ringbuffer_T *buffer, int64_t time,
  ...
  template <typename Ringbuffer_T>
  static typename Ringbuffer_T::DataType interpolate(Ringbuffer_T *buffer,
                                                     int64_t time) {
...
struct InterpolatorLinear {
  ...
  interpolate(Ringbuffer_T *buffer, int64_t time,
    ...
    const real_t w1 = static_cast<real_t>(time - *it_before) /
...
public:
  ...
  Ringbuffer()
      : times_(timering_t(times_raw_.data(), times_raw_.data() + Size,
                          times_raw_.data(), 0)) {}
  ...
  inline void insert(time_t stamp, const DataType &data) {
  ...
  TimeDataRangePair getBetweenValuesInterpolated(time_t stamp_from,
                                                 time_t stamp_to);
  ...
  inline void removeDataOlderThan(real_t seconds) {
    ...
    removeDataBeforeTimestamp_impl(times_.back() - secToNanosec(seconds));
  }
...
protected:
  ...
  inline DataType
  dataAtTimeIterator(typename timering_t::const_iterator iter) const {
    //! @todo: i believe this is wrong.
    return data_.col(iter.container_index());
  }
