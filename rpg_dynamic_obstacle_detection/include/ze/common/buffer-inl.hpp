...
template <typename Scalar, int Dim>
std::pair<Eigen::Matrix<Scalar, Dim, 1>, bool>
Buffer<Scalar, Dim>::getOldestValue() const {
  std::lock_guard<std::mutex> lock(mutex_);
  if (buffer_.empty()) {
...
template <typename Scalar, int Dim>
std::pair<Eigen::Matrix<Scalar, Dim, 1>, bool>
Buffer<Scalar, Dim>::getNewestValue() const {
  std::lock_guard<std::mutex> lock(mutex_);
  if (buffer_.empty()) {
    return std::make_pair(Vector(), false);
  }
...
Buffer<Scalar, Dim>::getOldestAndNewestStamp() const {
  std::lock_guard<std::mutex> lock(mutex_);
  if (buffer_.empty()) {
...
Buffer<Scalar, Dim>::getBetweenValuesInterpolated(int64_t stamp_from,
                                                  int64_t stamp_to) {
  ...
  for (size_t i = 0; i < n; ++i) {
    if (i == 0) {
      ...
      values.col(i) =
          (1.0 - w) * it_from_before->second + w * it_from_after->second;
    } else if (i == n - 1) {
      stamps(i) = stamp_to;
      const double w =
      ...
      values.col(i) =
          (1.0 - w) * it_to_before->second + w * it_to_after->second;
    } else {
...
template <typename Scalar, int Dim>
typename Buffer<Scalar, Dim>::VectorBuffer::iterator
Buffer<Scalar, Dim>::iterator_equal_or_before(int64_t stamp) {
...
template <typename Scalar, int Dim>
typename Buffer<Scalar, Dim>::VectorBuffer::iterator
Buffer<Scalar, Dim>::iterator_equal_or_after(int64_t stamp) {
  DEBUG_CHECK(!mutex_.try_lock()) << "Call lock() before accessing data.";
